...
import { SCOPE_GROUPS, ensureConfigLayout, getAccountPaths, getConfigRoot, getDefaultAccountPaths, loadAccountsConfig, mkdirSecret, saveAccountsConfig, upsertAccount, validateAccountId, writeSecretFile, } from './config.js';
...
function valueToNumber(value, fallback) {
...
function getAttachmentAllowDir() {
    const raw = process.env.ATTACHMENT_DIR?.trim();
    if (!raw)
        return null;
    return path.resolve(raw);
}
function assertPathInsideAllowDir(filePath, allowDir) {
    const resolved = path.resolve(filePath);
    const rel = path.relative(allowDir, resolved);
    if (rel.startsWith('..') || path.isAbsolute(rel)) {
        throw new Error(`Attachment path "${filePath}" is outside the allowed directory (ATTACHMENT_DIR=${allowDir}).`);
    }
}
function valueToAttachmentArray(value) {
    ...
    return value.flatMap((item) => {
        ...
        if (!filePath)
            return [];
        if (!allowDir) {
            throw new Error('Attachments are disabled: set ATTACHMENT_DIR to an absolute directory path to allow file attachments.');
        }
...
function driveFileDateForSort(file) {
...
function formatEmailItem(email, includeBody, bodyChars = DEFAULT_BODY_CHARS) {
    ...
    if (email.attachments.length > 0) {
        ...
        for (const att of email.attachments) {
            lines.push(`  - ${att.filename} (${att.contentType}, ${att.sizeBytes} bytes) — attachment_id: ${att.id}`);
        }
        lines.push(`  → To read attachment content: call get_all_attachments with account="${email.accountId}" email_id="${email.id}"`);
    }
    return lines.join('\n');
}
function formatAttachmentContent(content) {
    const lines = [
...
function formatDriveFileItem(file) {
    const lines = [
...
function formatDriveFileDetail(file) {
    const lines = [
        `**Account**: ${file.accountId} (${file.accountEmail})`,
        `**File ID**: ${file.id}`,
        `**Name**: ${file.name}`,
    ...
    if (file.parents?.length)
        lines.push(`**Parent IDs**: ${file.parents.join(', ')}`);
    if (file.webViewLink)
        lines.push(`**Open**: ${file.webViewLink}`);
...
function formatSpreadsheetMetadata(meta) {
    const lines = [
...
function formatCalendarInfo(cal) {
    ...
    if (cal.timeZone)
        lines.push(`**Time Zone**: ${cal.timeZone}`);
    if (cal.description)
...
function formatCalendarEvent(event) {
    ...
    if (event.htmlLink)
        lines.push(`**Open**: ${event.htmlLink}`);
...
class GmailMultiInboxServer {
    ...
    setupHandlers() {
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                {
                    name: 'list_accounts',
                    description: 'List all configured Gmail inbox accounts and their authentication/health status.',
                    inputSchema: {
                        type: 'object',
                        properties: {},
                ...
                {
                    name: 'read_emails',
                    description: 'Read emails from one account or aggregate across all enabled accounts when account is omitted.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            account: {
                ...
                {
                    name: 'search_emails',
                    description: 'Search Gmail using query syntax. Omitting account searches all enabled inboxes and merges results.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            account: {
                ...
                },
                {
                    ...
                    description: 'Search Google Drive file metadata. Omitting account searches all enabled accounts and merges results.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            account: {
                ...
                {
                    name: 'block_sender',
                    description: 'Block a sender by creating a Gmail filter (same mechanism as Gmail\'s native "Block" button). Optionally also trashes existing mail from that sender.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            account: { type: 'string', description: 'Account id.' },
                            sender: {
                                type: 'string',
                                description: 'Email address (e.g. "spam@foo.com"), domain ("@foo.com"), or Gmail search fragment ("from:x subject:promo").',
                            },
                            action: {
                                type: 'string',
                                enum: ['trash', 'archive', 'spam'],
                                description: 'What to do with matching mail. "trash" (default) mirrors Gmail\'s Block button.',
                                default: 'trash',
                            },
                            also_trash_existing: {
                                type: 'boolean',
                                description: 'If true (default), retroactively trashes up to 100 existing messages from this sender.',
                                default: true,
                            },
                        },
                ...
                },
                {
                    ...
                    description: 'List all Gmail filters for an account. Use to find filter_id for unblock_sender, or to audit what is being auto-trashed/archived/spammed.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            account: { type: 'string', description: 'Account id.' },
                ...
                {
                    name: 'unblock_sender',
                    description: 'Remove a Gmail filter. Specify either filter_id (exact match) or sender (matches filters whose "from" criteria equals this value).',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            account: { type: 'string', description: 'Account id.' },
                            ...
                            sender: {
                                type: 'string',
                                description: 'Sender string to match against filters\' from criteria. Used when filter_id is not provided.',
                            },
                        },
                        required: ['account'],
                        additionalProperties: false,
                ...
                },
                {
                    ...
                    description: 'Unsubscribe from a mailing list by invoking the List-Unsubscribe header (RFC 2369/8058). Same mechanism as Gmail\'s native "Unsubscribe" button. Falls back to mailto:. Returns method used.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            account: { type: 'string', description: 'Account id.' },
                            ...
                            dry_run: {
                                type: 'boolean',
                                description: 'If true, report what method would be used without executing the unsubscribe. Default false.',
                                default: false,
                            },
                        },
                ...
                {
                    name: 'mute_thread',
                    description: 'Mute a conversation: archives the thread now and (with scope="subject") installs a subject-based filter so future replies auto-archive. Approximates Gmail\'s client-side mute.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            account: { type: 'string', description: 'Account id.' },
                            ...
                            scope: {
                                ...
                                description: '"subject" (default) archives now + auto-archives future replies with the same subject. "thread_only" just archives this thread without creating a filter.',
                                default: 'subject',
                            },
                        },
                        required: ['account', 'thread_id'],
                ...
                },
                {
                    ...
                    description: 'Start OAuth onboarding for an account. Accepts credentials JSON or a path to credentials.json.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            account_id: {
                ...
                },
                {
                    ...
                    description: 'Complete OAuth onboarding by exchanging authorization code and storing token.json.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            account_id: {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                return textResult(`Error executing ${name}: ${error.message}`);
            }
        });
    }
    async loadConfig() {
    ...
    async handleListAccounts() {
        ...
        const healthList = await Promise.all(config.accounts.map((account) => getAccountHealth(this.configRoot, account)));
        const lines = ['# Gmail Accounts', '', `**Config Root**: ${this.configRoot}`, ''];
    ...
    async handleReadEmails(rawArgs) {
        const args = {
        ...
        const accountResults = await Promise.all(targetAccounts.map(async (account) => {
            try {
                ...
                return {
                    account,
                    emails: [],
                    error: `${account.id}: ${error.message}`,
                };
            }
        }));
        ...
        return textResult(formatEmailListOutput({
            title: 'Gmail Emails',
    ...
    async handleSearchEmails(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account, '').trim() || undefined,
            query: valueToString(rawArgs.query, ''),
    ...
    async handleSearchDriveFiles(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account, '').trim() || undefined,
            query: valueToString(rawArgs.query, ''),
            max_results: valueToNumber(rawArgs.max_results, 25),
        ...
        const accountResults = await Promise.all(targetAccounts.map(async (account) => {
            try {
                ...
                return { account, files, error: null };
            }
            catch (error) {
                return {
                    account,
        ...
        const errors = accountResults
            ...
            .filter((error) => Boolean(error));
        const returned = merged.slice(0, maxResults);
        return textResult(formatDriveFileListOutput({
    ...
    async handleGetLabels(rawArgs) {
        ...
        labels.forEach((label, index) => {
            lines.push(`${index + 1}. ${label.name} | id=${label.id} | type=${label.type ?? 'unknown'} | messages=${label.messagesTotal ?? 0}`);
        });
        return textResult(lines.join('\n'));
    }
    async handleMarkAsRead(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account),
    ...
    async handleAddLabels(rawArgs) {
        ...
        return textResult(`✅ Added label(s) to ${updated} message(s) in account ${account.id}. Labels: ${args.label_ids.join(', ')}`);
    }
    async handleRemoveLabels(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account),
            message_ids: valueToStringArray(rawArgs.message_ids),
            label_ids: valueToStringArray(rawArgs.label_ids),
        ...
        return textResult(`✅ Removed label(s) from ${updated} message(s) in account ${account.id}. Labels: ${args.label_ids.join(', ')}`);
    }
    async handleArchiveEmails(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account),
            message_ids: valueToStringArray(rawArgs.message_ids),
        };
    ...
    async handleTrashEmails(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account),
            message_ids: valueToStringArray(rawArgs.message_ids),
        };
    ...
    async handleCreateLabel(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account),
        ...
        const account = resolveWriteAccount(config, args.account);
        const client = await this.getClientForAccount(account);
        const label = await client.createLabel(args.name, args.label_list_visibility, args.message_list_visibility);
        return textResult(`✅ Created label in account ${account.id}: ${label.name} (id=${label.id}, type=${label.type ?? 'unknown'})`);
    }
    async handleDeleteLabel(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account),
    ...
    async handleBlockSender(rawArgs) {
        const rawAction = valueToString(rawArgs.action, 'trash').trim().toLowerCase();
        const action = rawAction === 'archive' || rawAction === 'spam' ? rawAction : 'trash';
        const args = {
            account: valueToString(rawArgs.account),
    ...
    async handleListBlockedSenders(rawArgs) {
        ...
        const lines = [`# Filters for account ${account.id}`, '', `**Total**: ${filters.length}`, ''];
        filters.forEach((filter, index) => {
            const c = filter.criteria ?? {};
            const a = filter.action ?? {};
            const criteriaParts = [];
            if (c.from)
                criteriaParts.push(`from: ${c.from}`);
            if (c.to)
                criteriaParts.push(`to: ${c.to}`);
            if (c.subject)
                criteriaParts.push(`subject: ${c.subject}`);
            if (c.query)
                criteriaParts.push(`query: ${c.query}`);
            if (c.hasAttachment)
                criteriaParts.push('hasAttachment: true');
            const actionParts = [];
            if (a.addLabelIds?.length)
                actionParts.push(`add: ${a.addLabelIds.join(', ')}`);
            if (a.removeLabelIds?.length)
                actionParts.push(`remove: ${a.removeLabelIds.join(', ')}`);
            if (a.forward)
                actionParts.push(`forward: ${a.forward}`);
    ...
    async handleUnblockSender(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account),
        ...
        if (matches.length === 0) {
            return textResult(`No filters in account ${account.id} matched sender \`${senderNeedle}\`. Use list_blocked_senders to inspect.`);
        }
        const deletedIds = [];
        ...
        return textResult(`✅ Deleted ${deletedIds.length} filter(s) matching \`${senderNeedle}\` in account ${account.id}: ${deletedIds.join(', ')}.`);
    }
    async handleUnsubscribeFromEmail(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account),
        ...
        if (httpsUrl && isOneClick) {
            if (args.dry_run) {
                return textResult(`[dry_run] Would POST one-click unsubscribe to ${httpsUrl}`);
            }
            try {
        ...
        if (mailtoUrl) {
            ...
            if (args.dry_run) {
                return textResult(`[dry_run] Would send mailto unsubscribe to ${mailto.to} (subject: "${mailto.subject}").`);
            }
            try {
    ...
    async handleMuteThread(rawArgs) {
        const rawScope = valueToString(rawArgs.scope, 'subject').trim().toLowerCase();
        const scope = rawScope === 'thread_only' ? 'thread_only' : 'subject';
        const args = {
            account: valueToString(rawArgs.account),
        ...
        if (scope === 'subject') {
            ...
            if (isGenericSubject(cleaned)) {
                ...
                const filter = await client.createFilter({ subject: cleaned }, { removeLabelIds: ['INBOX'] });
        ...
        if (result.filter_id)
            lines.push(`- Filter id: ${result.filter_id}`);
        if (result.subject_matched)
            lines.push(`- Subject matched: "${result.subject_matched}"`);
        if (result.warning)
    ...
    async handleDeleteDrafts(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account),
    ...
    async handleListDrafts(rawArgs) {
        const accountId = valueToString(rawArgs.account);
        ...
        const lines = drafts.map((d, i) => `${i + 1}. draft_id=${d.draftId} to="${d.to}" subject="${d.subject}" thread_id=${d.threadId ?? '(none)'} internalDate=${d.internalDate}`);
        return textResult([`✅ ${drafts.length} draft(s) in account ${account.id} (sorted oldest-first):`, ...lines].join('\n'));
    }
    async handleSearchDrafts(rawArgs) {
        const accountId = valueToString(rawArgs.account);
        ...
        const lines = drafts.map((d, i) => `${i + 1}. draft_id=${d.draftId} to="${d.to}" subject="${d.subject}" thread_id=${d.threadId ?? '(none)'} snippet="${d.snippet}"`);
    ...
    async handleBeginAccountAuth(rawArgs) {
    ...
    async handleFinishAccountAuth(rawArgs) {
        const args = {
            account_id: valueToString(rawArgs.account_id),
    ...
    async handleGetAttachment(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account),
        ...
        const account = resolveWriteAccount(config, args.account);
        const client = await this.getClientForAccount(account);
        const { bytes, metadata } = await client.getAttachment(args.email_id, args.attachment_id);
    ...
    async handleGetAllAttachments(rawArgs) {
        const args = {
            account: valueToString(rawArgs.account),
            email_id: valueToString(rawArgs.email_id),
        };
    ...
    // ─── Drive handlers ────────────────────────────────────────────────────────
    async handleListDriveFiles(rawArgs) {
        ...
        const results = await Promise.all(targetAccounts.map(async (account) => {
            try {
                ...
                return { account, files: result.files, nextPageToken: result.nextPageToken, error: null };
            }
            catch (error) {
                return { account, files: [], nextPageToken: undefined, error: `${account.id}: ${error.message}` };
            }
        }));
    ...
    async handleGetDriveFile(rawArgs) {
    ...
    async handleGetDriveFileContent(rawArgs) {
        const account = valueToString(rawArgs.account);
        const fileId = valueToString(rawArgs.file_id).trim();
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        ...
        const fakeMetadata = { id: fileId, filename, contentType, sizeBytes: bytes.length, isInline: false };
        ...
        if (content.textTruncated)
            lines.push('**Text Truncated**: true (file saved in full)');
        if (content.extractionError)
            lines.push(`**Extraction Error**: ${content.extractionError}`);
        if (content.text) {
            ...
            lines.push('', '*(no text extractable — file saved to disk)*');
        }
        return textResult(lines.join('\n'));
    }
    async handleUploadDriveFile(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
    ...
    async handleCreateDriveFolder(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        const folder = await client.createDriveFolder(valueToString(rawArgs.name), valueToString(rawArgs.parent_id, '').trim() || undefined);
        return textResult([
            '✅ Folder created.',
            '',
    ...
    async handleUpdateDriveFile(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
    ...
    async handleTrashDriveFile(rawArgs) {
        const account = valueToString(rawArgs.account);
        const fileId = valueToString(rawArgs.file_id).trim();
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
    ...
    async handleShareDriveFile(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
    ...
    async handleSheetsRead(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        const result = await client.readSheetValues(valueToString(rawArgs.spreadsheet_id), valueToString(rawArgs.range), valueToString(rawArgs.value_render_option, 'FORMATTED_VALUE') || 'FORMATTED_VALUE');
        const lines = [
            '# Sheets Values',
            '',
            `**Account**: ${acc.id} (${acc.email})`,
    ...
    async handleSheetsWrite(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        if (!Array.isArray(rawArgs.values))
            throw new Error('values must be a 2D array.');
        const values = rawArgs.values;
        const result = await client.writeSheetValues(valueToString(rawArgs.spreadsheet_id), valueToString(rawArgs.range), values, valueToString(rawArgs.value_input_option, 'USER_ENTERED') || 'USER_ENTERED');
        return textResult([
    ...
    async handleSheetsAppend(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        if (!Array.isArray(rawArgs.values))
            throw new Error('values must be a 2D array.');
        const values = rawArgs.values;
        const result = await client.appendSheetValues(valueToString(rawArgs.spreadsheet_id), valueToString(rawArgs.range), values);
        return textResult([
            '✅ Rows appended.',
            `Account: ${acc.id} (${acc.email})`,
            `Updated Range: ${result.updatedRange}`,
    ...
    async handleSheetsCreate(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
    ...
    async handleSheetsAddTab(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        const result = await client.addSheetTab(valueToString(rawArgs.spreadsheet_id), valueToString(rawArgs.title), rawArgs.index !== undefined ? valueToNumber(rawArgs.index, 0) : undefined);
    ...
    async handleSheetsRenameTab(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        await client.renameSheetTab(valueToString(rawArgs.spreadsheet_id), valueToString(rawArgs.current_title), valueToString(rawArgs.new_title));
        return textResult(`✅ Sheet tab renamed from "${valueToString(rawArgs.current_title)}" to "${valueToString(rawArgs.new_title)}".`);
    }
    async handleSheetsDeleteTab(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
    ...
    async handleSheetsFormat(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        await client.formatCells(valueToString(rawArgs.spreadsheet_id), valueToString(rawArgs.sheet_title), valueToString(rawArgs.range), {
            ...
            horizontalAlignment: (valueToString(rawArgs.horizontal_alignment, '').trim() || undefined),
            numberFormat: valueToString(rawArgs.number_format, '').trim() || undefined,
            wrapStrategy: (valueToString(rawArgs.wrap_strategy, '').trim() || undefined),
        });
        return textResult(`✅ Formatting applied to ${valueToString(rawArgs.range)} in sheet "${valueToString(rawArgs.sheet_title)}".`);
    }
    async handleSheetsAddChart(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        const result = await client.addChart(valueToString(rawArgs.spreadsheet_id), valueToString(rawArgs.sheet_title), {
            chartType: valueToString(rawArgs.chart_type),
    ...
    async handleSheetsInsertDimension(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        const dim = valueToString(rawArgs.dimension);
        const count = valueToNumber(rawArgs.count, 1);
        await client.insertDimension(valueToString(rawArgs.spreadsheet_id), valueToString(rawArgs.sheet_title), dim, valueToNumber(rawArgs.start_index, 0), count);
    ...
    async handleSheetsDeleteDimension(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        const dim = valueToString(rawArgs.dimension);
        const count = valueToNumber(rawArgs.count, 1);
        await client.deleteDimension(valueToString(rawArgs.spreadsheet_id), valueToString(rawArgs.sheet_title), dim, valueToNumber(rawArgs.start_index, 0), count);
    ...
    async handleDocsCreate(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
    ...
    async handleDocsAppend(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        await client.appendToDocument(valueToString(rawArgs.document_id), valueToString(rawArgs.text), {
            style: (valueToString(rawArgs.style, 'NORMAL_TEXT') || 'NORMAL_TEXT'),
            bold: rawArgs.bold !== undefined ? valueToBoolean(rawArgs.bold) : undefined,
            italic: rawArgs.italic !== undefined ? valueToBoolean(rawArgs.italic) : undefined,
        });
        return textResult(`✅ Text appended to document ${valueToString(rawArgs.document_id)} in account ${acc.id}.`);
    }
    async handleDocsReplaceText(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        const result = await client.replaceInDocument(valueToString(rawArgs.document_id), valueToString(rawArgs.find), valueToString(rawArgs.replace_with), valueToBoolean(rawArgs.match_case, false));
    ...
    async handleDocsInsertTable(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
    ...
    async handleDocsApplyStyle(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        await client.applyDocHeadingStyle(valueToString(rawArgs.document_id), valueToNumber(rawArgs.start_index, 0), valueToNumber(rawArgs.end_index, 0), valueToString(rawArgs.style));
    ...
    // ─── Calendar handlers ─────────────────────────────────────────────────────
    async handleListCalendars(rawArgs) {
        const accountId = valueToString(rawArgs.account, '').trim() || undefined;
        const config = await this.loadConfig();
        const targetAccounts = resolveReadAccounts(config, accountId);
        const results = await Promise.all(targetAccounts.map(async (account) => {
            try {
                ...
                return { cals, error: null };
            }
            catch (error) {
                return { cals: [], error: `${account.id}: ${error.message}` };
            }
        }));
        ...
        if (errors.length > 0) {
            lines.push('## Account Errors');
            lines.push(errors.map((e) => `- ${e}`).join('\n'));
        }
        return textResult(lines.join('\n'));
    }
    async handleListEvents(rawArgs) {
        const accountId = valueToString(rawArgs.account, '').trim() || undefined;
        ...
        const results = await Promise.all(targetAccounts.map(async (account) => {
            try {
                ...
                return { events: [], error: `${account.id}: ${error.message}` };
            }
        }));
        ...
        if (timeMin)
            lines.push(`**From**: ${timeMin}`);
        if (timeMax)
            lines.push(`**To**: ${timeMax}`);
        if (query)
    ...
    async handleCreateEvent(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        ...
        if (!startDT && !startD)
            throw new Error('Provide start_date_time or start_date.');
        if (!endDT && !endD)
            throw new Error('Provide end_date_time or end_date.');
        const event = await client.createCalendarEvent(valueToString(rawArgs.calendar_id, 'primary').trim() || 'primary', {
    ...
    async handleUpdateEvent(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
        ...
        const event = await client.updateCalendarEvent(valueToString(rawArgs.calendar_id), valueToString(rawArgs.event_id), {
            ...
            status: (valueToString(rawArgs.status, '').trim() || undefined),
            sendNotifications: rawArgs.send_notifications !== undefined ? valueToBoolean(rawArgs.send_notifications, true) : true,
        });
        return textResult(['✅ Calendar event updated.', '', formatCalendarEvent(event)].join('\n'));
    }
    async handleDeleteEvent(rawArgs) {
        const account = valueToString(rawArgs.account);
        const config = await this.loadConfig();
        const acc = resolveWriteAccount(config, account);
        const client = await this.getClientForAccount(acc);
...
function resolveTransportMode() {
    ...
    if (explicit === 'stdio' || explicit === 'sse')
        return explicit;
...
async function runSseServer() {
    ...
    const tokensEqual = (provided) => {
        if (!SECRET_TOKEN_BUF)
            return true;
        const providedBuf = Buffer.from(provided, 'utf8');
        if (providedBuf.length !== SECRET_TOKEN_BUF.length)
            return false;
    ...
    const requireTokenBootstrap = (req, res, next) => {
        if (!SECRET_TOKEN || req.method === 'OPTIONS') {
            next();
            return;
        }
...
loadAccounts();
    ...
    app.get('/', requireTokenBootstrap, (_req, res) => {
    ...
    app.post('/api/accounts/begin', requireToken, async (req, res) => {
        try {
            const { account_id, email, credentials_json, display_name, scope_groups } = req.body;
            ...
            const scopeGroups = Array.isArray(scope_groups)
                ? scope_groups.filter((s) => SCOPE_GROUPS.includes(s))
                : undefined;
