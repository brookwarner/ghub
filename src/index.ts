...
function formatEmailItem(
  email: ParsedEmail,
  includeBody: boolean,
  bodyChars: number | null = DEFAULT_BODY_CHARS,
): string {
  const lines = [
    ...
    input.returned.forEach((email, index) => {
      ...
      sections.push(
        formatEmailItem(
          email,
          input.includeBody,
          input.bodyChars === undefined ? DEFAULT_BODY_CHARS : input.bodyChars,
        ),
      );
      sections.push('');
    });
...
class GmailMultiInboxServer {
  ...
  private async handleGetAllAttachments(
    ...
    const { downloaded, errors: downloadErrors } = await client.getAllAttachments(
      args.email_id,
    );

    const settled = await Promise.allSettled(
      downloaded.map(({ bytes, metadata }) => saveAndExtract(bytes, metadata)),
    );

    const attachments: AttachmentContent[] = [];
    const errors: Array<{ attachment_id: string; error: string }> = downloadErrors.map(
      (e) => ({ attachment_id: e.metadata.filename, error: e.error }),
    );
  ...
  private async handleListTaskLists(rawArgs: Record<string, unknown>): Promise<CallToolResult> {
    const accountId = valueToString(rawArgs.account, '').trim() || undefined;
    const config = await this.loadConfig();
    const targetAccounts = resolveReadAccounts(config, accountId);
    ...
    const results = await Promise.all(
      targetAccounts.map(async (account) => {
        try {
          ...
          return { lists, error: null as string | null };
        } catch (error) {
          const msg = (error as Error).message;
          if (msg.includes('Insufficient Permission') || msg.includes('insufficientPermissions')) return { lists: [] as TaskList[], error: null };
          return { lists: [] as TaskList[], error: `${account.id}: ${msg}` };
        }
      })
    );
    ...
    if (all.length === 0) lines.push('No task lists found.');
    all.forEach((tl) => {
      lines.push(`- **${tl.title}** (id: \`${tl.id}\`) — ${tl.accountEmail}`);
    });
    if (errors.length > 0) { lines.push(''); lines.push('## Errors'); lines.push(errors.map((e) => `- ${e}`).join('\n')); }
    return textResult(lines.join('\n'));
  }

  private async handleListTasks(rawArgs: Record<string, unknown>): Promise<CallToolResult> {
    const accountId = valueToString(rawArgs.account, '').trim() || undefined;
    ...
    const results = await Promise.all(
      targetAccounts.map(async (account) => {
        try {
          ...
          return { tasks, error: null as string | null };
        } catch (error) {
          const msg = (error as Error).message;
          if (msg.includes('Insufficient Permission') || msg.includes('insufficientPermissions')) return { tasks: [] as Task[], error: null };
          return { tasks: [] as Task[], error: `${account.id}: ${msg}` };
        }
      })
    );
    ...
    if (all.length === 0) lines.push('No tasks found.');
    for (const t of all) {
  ...
  private async handleAddTask(rawArgs: Record<string, unknown>): Promise<CallToolResult> {
    const accountId = valueToString(rawArgs.account, '').trim() || undefined;
    const taskListId = valueToString(rawArgs.task_list_id);
  ...
  private async handleCompleteTask(rawArgs: Record<string, unknown>): Promise<CallToolResult> {
    const accountId = valueToString(rawArgs.account, '').trim() || undefined;
    const taskListId = valueToString(rawArgs.task_list_id);
  ...
  private async handleDeleteTask(rawArgs: Record<string, unknown>): Promise<CallToolResult> {
    const accountId = valueToString(rawArgs.account, '').trim() || undefined;
    const taskListId = valueToString(rawArgs.task_list_id);
    const taskId = valueToString(rawArgs.task_id);
    const config = await this.loadConfig();
  ...
  private async handleMoveTask(rawArgs: Record<string, unknown>): Promise<CallToolResult> {
    const accountId = valueToString(rawArgs.account, '').trim() || undefined;
    const taskListId = valueToString(rawArgs.task_list_id);
    const taskId = valueToString(rawArgs.task_id);
  ...
  async connectTransport(transport: StdioServerTransport | StreamableHTTPServerTransport): Promise<void> {
    await ensureConfigLayout(this.configRoot);
...
loadAccounts();
  ...
  app.get('/api/accounts', requireToken, async (_req: Request, res: Response) => {
    try {
  ...
  app.post('/api/accounts/finish', requireToken, async (req: Request, res: Response) => {
    try {
      const { account_id, authorization_code } = req.body as Record<string, unknown>;
  ...
  app.delete('/api/accounts/:id', requireToken, async (req: Request, res: Response) => {
    try {
      const configRoot = getConfigRoot();
      let config = await loadAccountsConfig(configRoot);
  ...
  app.all('/sse', requireToken, async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.error('[ghub] Session error: ' + (error as Error).message);
      if (!res.headersSent) res.status(500).type('text/plain').send((error as Error).message);
    }
  });
